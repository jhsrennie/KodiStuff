...
CRhsRegistry::CRhsRegistry(const char* Key, BOOL HKeyLocalMachine)
{
  m_HKeyLocalMachine = FALSE;
  lstrcpy(m_Key, "");
...
BOOL CRhsRegistry::SetKey(const char* Key, BOOL HKeyLocalMachine)
{
  if (!Key)
    return(FALSE);
...
BOOL CRhsRegistry::GetKey(char* Key, int Len, BOOL* HKeyLocalMachine)
{
...
BOOL CRhsRegistry::WriteString(const char* Key, const char* String)
{ HKEY h;
  LONG l;
  DWORD d;
...
// Write to the registry
  ...
  l = RegSetValueEx(h, Key, 0, REG_SZ, (CONST BYTE*) String, lstrlen(String) + 1);
  RegCloseKey(h);
...
BOOL CRhsRegistry::WriteInt(const char* Key, int Value)
{ HKEY h;
  LONG l;
  DWORD value, d;
...
const char* CRhsRegistry::GetString(const char* Key, const char* Default, int Len)
{ DWORD len;
  HKEY h;
  LONG l;
  DWORD regtype, d;
...
// An internal 4 string buffer is used to store read strings
  ...
  static int  id = -1;
  static char string[4][LEN_REGISTRYPATH+1];
...
int CRhsRegistry::GetInt(const char* Key, int Default)
{ HKEY h;
  LONG l;
